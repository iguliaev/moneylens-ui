...
test.describe("User Data Isolation", () => {
  ...
  test.describe("Dashboard Isolation", () => {
    test("User A sees only their own totals on dashboard", async ({ page }) => {
      ...
      await expect(page.getByTestId("dashboard-spent-total")).toContainText("100");
      await expect(page.getByTestId("dashboard-earned-total")).toContainText("500");
      await expect(page.getByTestId("dashboard-saved-total")).toContainText("200");

      // Verify User A's categories appear, not User B's
      await expect(
        page.getByTestId("dashboard-category-name").filter({ hasText: "userA-Groceries" })
      ).toBeVisible();
      await expect(
        page.getByTestId("dashboard-category-name").filter({ hasText: "userB-Groceries" })
      ).not.toBeVisible();
    });
  ...
  test.describe("Transaction Pages Isolation", () => {
    test("Spend page shows only current user's transactions", async ({ page }) => {
      await loginUser(page, userA.email, userA.password);
      await page.goto("/spend");

      // User A's transaction visible
      await expect(
        page.getByTestId("spend-row-notes").filter({ hasText: "userA-spend-transaction" })
      ).toBeVisible();

      // User B's transaction NOT visible
      await expect(
        page.getByTestId("spend-row-notes").filter({ hasText: "userB-spend-transaction" })
      ).not.toBeVisible();

      // Total should reflect only User A's spend (100)
      await expect(page.getByTestId("spend-total-current-month-value")).toContainText("100");
    });

    test("Earn page shows only current user's transactions", async ({ page }) => {
      await loginUser(page, userA.email, userA.password);
      await page.goto("/earn");

      await expect(
        page.getByTestId("earn-row-notes").filter({ hasText: "userA-earn-transaction" })
      ).toBeVisible();
      await expect(
        page.getByTestId("earn-row-notes").filter({ hasText: "userB-earn-transaction" })
      ).not.toBeVisible();

      await expect(page.getByTestId("earn-total-current-month-value")).toContainText("500");
    });

    test("Save page shows only current user's transactions", async ({ page }) => {
      await loginUser(page, userA.email, userA.password);
      await page.goto("/save");

      await expect(
        page.getByTestId("save-row-notes").filter({ hasText: "userA-save-transaction" })
      ).toBeVisible();
      await expect(
        page.getByTestId("save-row-notes").filter({ hasText: "userB-save-transaction" })
      ).not.toBeVisible();

      await expect(page.getByTestId("save-total-current-month-value")).toContainText("200");
    });
  });

  test.describe("Settings Pages Isolation", () => {
    test("Categories page shows only current user's categories", async ({ page }) => {
      await loginUser(page, userA.email, userA.password);
      ...
      await expect(
        page.getByTestId("categories-row").filter({ hasText: "userA-Groceries" })
      ).toBeVisible();

      // User B's category NOT visible
      await expect(
        page.getByTestId("categories-row").filter({ hasText: "userB-Groceries" })
      ).not.toBeVisible();
    });
    ...
    test("Bank accounts page shows only current user's accounts", async ({ page }) => {
      await loginUser(page, userA.email, userA.password);
