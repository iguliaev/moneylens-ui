...
test.describe('Transactions', () => {
  ...
  test('user can create spend transaction', async ({ page }) => {
    ...
    await page.getByTestId('spend-form-category').selectOption({ label: 'Groceries' });
  ...
  test('user can edit transaction', async ({ page }) => {
    ...
    await expect(page.getByTestId('spend-row-notes').filter({ hasText: 'Original transaction' })).toBeVisible();
    ...
    await expect(page.getByTestId('spend-row-amount').filter({ hasText: '150.00' })).toBeVisible();
    await expect(page.getByTestId('spend-row-notes').filter({ hasText: 'Updated transaction' })).toBeVisible();
    
    // Verify old values don't appear
    await expect(page.getByTestId('spend-row-amount').filter({ hasText: '100.00' })).not.toBeVisible();
  ...
  test('user can delete transaction', async ({ page }) => {
    ...
    // Wait for transaction to appear
    const notesCell = page.getByTestId('spend-row-notes').filter({ hasText: 'To be deleted' });
    await expect(notesCell).toBeVisible();
    const amountCell = page.getByTestId('spend-row-amount').filter({ hasText: '75.00' });
    await expect(amountCell).toBeVisible();
