...
export default function SavePage() {
  ...
  tags: form.tags.length ? form.tags : null,
      ...
      setError(e?.message ?? "Failed to create saving");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function saveEdit() {
    ...
    try {
      ...
      setError(e?.message ?? "Failed to update saving");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Save — {monthLabel}</h1>
      ...
      <section className="space-y-4">
        <form onSubmit={handleCreate} className="border rounded p-4 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
          ...
          <div className="md:col-span-6 flex justify-end">
            <button className="px-3 py-1 rounded border" disabled={saving}>{saving ? "Saving…" : "Add Saving"}</button>
          </div>
        </form>
